    ...
    for (pft.i in seq(pft.samples)) {
      ...
      for (trait.i in seq(pft.samples[[pft.i]])) {
        ...
        if (names(pft.samples[[pft.i]])[trait.i] %in% param.names[[pft.i]]) {
          ...
          ensemble.samples[[pft.i]][, trait.i] <- stats::quantile(
            pft.samples[[pft.i]][[trait.i]],
            random.samples[, col.i]
          )
          sampled.indices[[pft.i]][, trait.i] <- sapply(sampled.values, function(val) {
            which.min(abs(trait.values - val))
          })
        }
      }
    ...
    for (i in seq_len(ensemble.size)) {
      ...
      for (input_i in seq_along(settings$run$inputs)) {
        ...
        if (!is.null(samples[[input_tag]])) {
          ...
          input_info <- paste0(
            input_info, format(input_tag, width = 12, justify = "left"), ": ",
            samples[[input_tag]]$samples[[i]], "\n"
          )
        }
      }
    ...
    for (i in seq_len(ensemble.size)) {
      ...
      do.call(my.write_restart,
        args = list(
          ...
          inputs = input_list,
          RENAME = rename
        ) # for restart from previous model runs, not sharing the same outdir
      )
    }
    params <- new.params
